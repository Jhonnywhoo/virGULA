...
# Removendo conteúdo de um arquivo
def remover_de_txt(arquivo, conteudo):
    with open(arquivo, 'r') as file:
        linhas = file.readlines()
    
    with open(arquivo, 'w') as file:
        for linha in linhas:
            if linha.strip() != conteudo:
                file.write(linha)
...
def addCombo(cardapio, combo):
    chave = str(len(cardapio) + 1)
    cardapio[chave] = combo
    SaveTxt(cardapio)       

def remCombo(cardapio, chave):
    if chave in cardapio:
        del cardapio[chave]
        SaveTxt(cardapio)
    else:
...
def SaveTxt(Dict):
    with open("main.txt", 'w') as Dados:
        for chave, dados in Dict.items():
            Dados.write(f"{chave},{dados['Nome']},{dados['preço']},{dados['Lanche']},{dados['Acompanhamento']},{dados['Copo']}\n") 

def LoadTxt(tipo='todos'):
    L0 = []  # Lista que receberá TXT
    L1 = {}  # DCT que receberá a lista
    with open("main.txt", 'r') as Dados:
        for i in Dados:
            if tipo == 'todos':
                info = i.strip().split(",")
                L0.append({'Nome': info[1], 'preço': float(info[2]), 'Lanche': info[3], 'Acompanhamento': info[4], 'Copo': info}[5])
            elif tipo in i:
                info = i.strip().split(",")
                L0.append({'Nome': info[1], 'preço': float(info[2]), 'Lanche': info[3], 'Acompanhamento': info[4], 'Copo': info}[5])
        for i in L0:  # Transforma lista em dicionário
            L1[i[0]] = i
    return L1                   
